...
function bindModsToggleSwitch(){
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onchange = () => {
            const modElement = document.getElementById(v.getAttribute('formod'))
            if(v.checked) {
                modElement.setAttribute('enabled', '')
            } else {
                if(modElement.hasAttribute('reqmod')) {
                    // If it's a required mod, show a confirmation dialog
                    setOverlayContent(
                        Lang.queryJS('settings.mods.requiredModDisableTitle'),
                        Lang.queryJS('settings.mods.requiredModDisableMessage'),
                        Lang.queryJS('settings.mods.requiredModDisableConfirm'),
                        Lang.queryJS('settings.mods.requiredModDisableCancel')
                    )
                    setOverlayHandler(() => {
                        toggleOverlay(false)
                        v.checked = true // Re-enable the checkbox
                    })
                    setDismissHandler(() => {
                        toggleOverlay(false)
                    })
                    ...
                    modElement.removeAttribute('enabled')
                }
            }
        }
    })
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/vialolis/MysticOdysseyLauncher/releases.atom',
        success: (data) => {
